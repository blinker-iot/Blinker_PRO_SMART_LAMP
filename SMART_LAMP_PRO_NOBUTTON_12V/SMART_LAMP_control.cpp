...
static uint8_t  r_set = 0;
static uint8_t  g_set = 0;
static uint8_t  b_set = 0;
static uint8_t  w_set = 0;
...
void stripInit()
{
    pinMode(BLINKER_R_PIN, OUTPUT);
    pinMode(BLINKER_G_PIN, OUTPUT);
    pinMode(BLINKER_B_PIN, OUTPUT);
    pinMode(BLINKER_W_PIN, OUTPUT);

    analogWriteRange(255);

    analogWrite(BLINKER_R_PIN, 0);
    analogWrite(BLINKER_G_PIN, 0);
    analogWrite(BLINKER_B_PIN, 0);    
    analogWrite(BLINKER_W_PIN, 0);
}

void setStripColor(uint8_t _r, uint8_t _g, uint8_t _b)
{
    // analogWrite(BLINKER_R_PIN, _r);
    // analogWrite(BLINKER_G_PIN, _g);
    // analogWrite(BLINKER_B_PIN, _b);
...
void setStripColor(uint32_t _c)
{
    // analogWrite(BLINKER_R_PIN, _c >> 16 & 0xFF);
    // analogWrite(BLINKER_G_PIN, _c >>  8 & 0xFF);
    // analogWrite(BLINKER_B_PIN, _c       & 0xFF);
...
void setStripBright(uint8_t bright)
{
    bright_set = bright;
} 
...
void stripShow()
{
    analogWrite(BLINKER_R_PIN, r_set * bright_set / 255);
    analogWrite(BLINKER_G_PIN, g_set * bright_set / 255);
    analogWrite(BLINKER_B_PIN, b_set * bright_set / 255);
}
...
void rainbowDisplay()
{
    ...
    setStripColor(Wheel(lampStep & 255));
    stripShow();
    lampStep = (lampStep + 1) & 0xFF;
...
uint32_t breath()
{
    ...
    setStripColor(r, g, b);
    setStripBright(brtLumi * lum / 256);
    stripShow();
...
{
    if (_time > 5000) _time = 5000;
    
    uint8_t lum = map(_time, 0, 5000, 255, 0);
    uint8_t r   = 255 - lum;
    uint8_t g   = lum;
    uint8_t b   = lum;
...
void setStandard(uint32_t color)
{
    ...
    latest_color = r_set << 16 | g_set << 8 | b_set;
